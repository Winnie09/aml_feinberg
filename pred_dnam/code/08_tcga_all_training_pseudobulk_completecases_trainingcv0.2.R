...
# ---------------------------------------
# highly variable CpG in training samples
# ---------------------------------------
cm <- rowMeans(d)
cv <- sqrt(rowMeans(d * d - c^2) / (ncol(d) - 1) * ncol(d))
d <- d[cv >=0.2, ]
...
saveRDS(m, '/home-4/whou10@jhu.edu/scratch/Wenpin/aml_feinberg/pred_dnam/result/pseudobulk/tcga_training/laml_trainmodel_completecase_trainingcv0.2.rds')

## predict
pred <- predict(predexpr, m)
saveRDS(pred, '/home-4/whou10@jhu.edu/scratch/Wenpin/aml_feinberg/pred_dnam/result/pseudobulk/tcga_training/all_pred_completecase_trainingcv0.2.rds')
...
saveRDS(list(crosssample = cv1, crosssite = cv2), '/home-4/whou10@jhu.edu/scratch/Wenpin/aml_feinberg/pred_dnam/result/pseudobulk/tcga_training/all_perf_completecase_trainingcv0.2.rds')
